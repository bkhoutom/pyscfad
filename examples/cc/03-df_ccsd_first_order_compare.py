"""Compare two SCF-gradient backends inside a canonical DF-CCSD calculation.

This example evaluates the same density-fitted CCSD energy and nuclear
gradient twice:
...
print()
print("Example: canonical DF-CCSD energy and gradient with two SCF backpropagation routes")
print("What is being evaluated: density-fitted CCSD on the water dimer")
...
if diff["grad_max_abs"] < 1e-5 and abs(diff["energy_diff"]) < 1e-6:
    print("Interpretation: the implicit and custom first-order CPHF results agree closely, so the custom route is reproducing the implicit CCSD gradient well on this example.")
else:

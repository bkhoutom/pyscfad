...
def _build_static_dlno_topology():
    ...
    coefficient matrices.  Inside ``jax.value_and_grad`` those would be
    tracers, so the topology must be built outside the AD pass.  Only
    ``rebuild_dlno_prescreen_data`` (called inside the energy function) is
    tracer-safe.
    """
    seed_mol = build_mol()

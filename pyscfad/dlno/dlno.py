...
def kernel(mydlno, auxbasis=None,
    ...
    ep_domains = []
    for i in range(nocc):
        idx = np.where(abs(E1[i]) > pair_energy_thr)[0]
    ...
    for atmlst, lmo_idx in unique_ep_domains.items():
        ...
        for atms1, idx1 in unique_sub_e_domains.items():
            ...
            for ij_idx, i_idx in unique_strong_pairs.items():
                ...
                _mp2 = mp2.kernel_dfmp2(fake_mf, orbloc.T.conj() @ s22 @ lmo_ij_canon, with_t2=False)
                emp2 = _mp2[0] if isinstance(_mp2, tuple) else _mp2

...
def get_bp_domain(mol, mos, s1e=None, bp_thr=0.999,
    ...
    for i in range(nmo):
        orbi = mos[:,i]
        PS = jnp.outer(orbi, orbi) * jnp.asarray(s1e)
        GOP = jnp.sum(PS, axis=1)

        q = jnp.abs(jnp.asarray([GOP[slice(*aoslices[a])].sum() for a in atmlst]))

        _atms = atmlst[q > q_thr]
        av = _compute_av(mol, orbi, s1e, _atms)

        if av < bp_thr:
            center_id = int(jnp.argsort(-q)[0])
            _sorted_atm_idx = np.asarray(jnp.argsort(rr[center_id]))
...
def atom_distance(mol, atmlst=None):
    ...
    if atmlst is None:
        atmlst = np.arange(mol.natm)
    coords = jnp.asarray(mol.atom_coords())[jnp.asarray(atmlst)].reshape(-1,3)

...
static double lnoccsdt_get_energy_lt_ulo(double *ulo, int nlo,
                                         double *w, double *v,
                                         double *mo_energy,
                                         double *lt_t, double *lt_w, int nlap,
                                         int nocc, int a, int b, int c,
                                         double fac, double *cache)
{
        ...
        for (x = 0; x < nlo; x++) {
                ...
                for (k = 0; k < nocc; k++) {
                        double s = 0;
                        for (l = 0; l < nocc; l++) {
                                s += ulo[x*nocc+l] * w[(l*nocc+j)*nocc+k];
                        }
                        wu[j*nocc+k] = s;
                } }
                ...
                for (q = 0; q < nlap; q++) {
                        ...
                        for (i = 0; i < nocc; i++) {
                                double ui = ulo[x*nocc+i] * exp(tq * mo_energy[i]);
                                for (j = 0; j < nocc; j++) {
                                        double uij = ui * exp(tq * mo_energy[j]);
                                        for (k = 0; k < nocc; k++) {
                                                et += qfac
                                                    * uij * exp(tq * mo_energy[k])
                                                    * wu[j*nocc+k]
                                                    * v[(i*nocc+j)*nocc+k];
                                        }
                                }
...
static double contract6_lt_ulo(int nocc, int nvir, int a, int b, int c,
                               double *ulo, int nlo,
                               double *lt_t, double *lt_w, int nlap,
                               double *mo_energy, double *t1T, double *t2T,
                               double *fvo, double *vooo, double *cache1,
                               void **cache, int *permute_idx, double fac)
{
        int nooo = nocc * nocc * nocc;
        int *idx0 = permute_idx;
        int *idx1 = idx0 + nooo;
        ...
        double et;
        if (b == c) {
                et = lnoccsdt_get_energy_lt_ulo(ulo, nlo, w0, z0,
                                                mo_energy, lt_t, lt_w, nlap,
                                                nocc, a, b, c, .5, wu);
        } else {
                et = lnoccsdt_get_energy_lt_ulo(ulo, nlo, w0, z0,
                                                mo_energy, lt_t, lt_w, nlap,
...
void lnoccsdt_contract_lt_ulo(double *e_tot, double *ulo, int nlo,
                              double *lt_t, double *lt_w, int nlap,
                              double *mo_energy, double *t1T, double *t2T,
                              double *vooo, double *fvo,
                              int nocc, int nvir, int a0, int a1,
                              void *cache)
{
        ...
        _make_permute_indices(permute_idx, nocc);
#pragma omp parallel default(none) \
        shared(njobs, nocc, nvir, ulo, nlo, lt_t, lt_w, nlap, mo_energy, \
               t1T, t2T, vooo, fvo, jobs, e_tot, permute_idx)
{
        int a, b, c;
        size_t k;
        double *cache1 = malloc(sizeof(double) *
                                (nocc*nocc*nocc*3 + nocc*nocc + 2));
        double *t1Thalf = malloc(sizeof(double) * nvir*nocc * 2);
        double *fvohalf = t1Thalf + nvir*nocc;
        for (k = 0; k < nvir*nocc; k++) {
                t1Thalf[k] = t1T[k] * .5;
...
#pragma omp for schedule (dynamic, 4)
        for (k = 0; k < njobs; k++) {
                ...
                b = jobs[k].b;
                c = jobs[k].c;
                e += contract6_lt_ulo(nocc, nvir, a, b, c, ulo, nlo,
                                      lt_t, lt_w, nlap, mo_energy,
